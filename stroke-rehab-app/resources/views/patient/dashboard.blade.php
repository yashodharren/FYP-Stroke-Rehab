        ...
        <form method="POST" action="{{ route('patient.plan-feedback', $activePlan->id) }}" class="p-6">
            ...
            <div class="space-y-6">
                ...
                <div class="border border-gray-200 rounded-lg p-4">
                    <input type="hidden" name="exercises[{{ $loop->index }}][plan_exercise_id]" value="{{ $planExercise->id }}">
                    <h4 class="font-semibold text-gray-900 mb-3">{{ $planExercise->exercise->name }}</h4>
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">Pain Level (0 = none, 10 = severe)</label>
                            <div class="flex gap-1">
                                @for($i = 0; $i <= 10; $i++)
                                    <label class="flex flex-col items-center cursor-pointer">
                                    <input type="radio" name="exercises[{{ $loop->parent->index }}][pain_level]" value="{{ $i }}" class="sr-only peer" @if($i===0) checked @endif>
                                    <span class="w-7 h-7 flex items-center justify-center rounded-full text-xs font-medium border border-gray-300 peer-checked:bg-red-500 peer-checked:text-white peer-checked:border-red-500 hover:bg-red-100 transition-colors">{{ $i }}</span>
                                    </label>
                                    @endfor
                            </div>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">Difficulty (1 = easy, 5 = very hard)</label>
                            <div class="flex gap-2">
                                @for($i = 1; $i <= 5; $i++)
                                    <label class="flex flex-col items-center cursor-pointer">
                                    <input type="radio" name="exercises[{{ $loop->parent->index }}][difficulty_rating]" value="{{ $i }}" class="sr-only peer" @if($i===3) checked @endif>
                                    <span class="w-9 h-9 flex items-center justify-center rounded-full text-sm font-medium border border-gray-300 peer-checked:bg-blue-500 peer-checked:text-white peer-checked:border-blue-500 hover:bg-blue-100 transition-colors">{{ $i }}</span>
                                    </label>
                                    @endfor
                            </div>
                        </div>
